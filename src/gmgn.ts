...
export function hasVolumeAtLeast(record: Record<string, unknown>, minVolume: number): boolean {
  const volume = typeof record.volume_24h === "number" ? record.volume_24h : Number(record.volume_24h);
...
// but already dex paid + bundler <= 20% + organic + low rug risk + volume >= config.GMGN_UNBOUNDED_MIN_VOLUME_24H.
